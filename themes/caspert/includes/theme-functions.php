...
function caspert_get_version_theme(): string
{
 ...
 */
function caspert_opt(string $class): string
{
 ...
 */
function caspert_get_field_tab_data(string $class_name, int $post_id = null): array
{
...
// set favicon default
if (!function_exists('caspert_fallback_favicon')) {
    add_action('wp_head', 'caspert_fallback_favicon');
    function caspert_fallback_favicon(): void
    {
        if (!has_site_icon()) {
...
// check is blog
function caspert_is_blog(): bool
{
    return (is_home() || (is_archive() && get_post_type() === 'post') || is_search());
}

// Callback Comment List
function caspert_comments($caspert_comment, $caspert_comment_args, $caspert_comment_depth): void
{
    if ($caspert_comment_args['style'] == 'div') :
        $caspert_comment_tag = 'div';
        $caspert_comment_add_below = 'comment';
    else :
        $caspert_comment_tag = 'li';
        $caspert_comment_add_below = 'div-comment';
    endif;

    ?>
    <<?php echo $caspert_comment_tag . ' ' ?><?php comment_class(empty($caspert_comment_args['has_children']) ? '' : 'parent') ?> id="comment-<?php comment_ID() ?>">

    <?php if ('div' != $caspert_comment_args['style']) : ?>
...
<?php endif; ?>
    <div class="author vcard">
        <div class="author__avatar">
            <?php if ($caspert_comment_args['avatar_size'] != 0) {
                echo get_avatar($caspert_comment, $caspert_comment_args['avatar_size']);
            } ?>
        </div>
    ...
    <?php if ($caspert_comment->comment_approved == '0') : ?>
    <div class="awaiting">
        <?php esc_html_e('Bình luận của bạn đang chờ kiểm duyệt.', 'caspert'); ?>
    </div>
<?php endif; ?>
    ...
    <?php if ($caspert_comment_args['style'] != 'div') : ?>
...
// Content Nav
function caspert_comment_nav(): void
{
    if (get_comment_pages_count() > 1 && get_option('page_comments')) :
        ?>
        <nav class="navigation comment-navigation">
            <h2 class="screen-reader-text">
                <?php esc_html_e('Điều hướng bình luận', 'caspert'); ?>
            </h2>

            <div class="nav-links">
                <?php
                if ($prev_link = get_previous_comments_link(esc_html__('Bình luận cũ hơn', 'caspert'))) :
                    printf('<div class="nav-previous">%s</div>', $prev_link);
                endif;

                if ($next_link = get_next_comments_link(esc_html__('Bình luận mới hơn', 'caspert'))) :
...
// Get col global
function caspert_col_use_sidebar($option_sidebar, $active_sidebar): string
{
...
function caspert_col_sidebar(): string
{
    return 'col-12 col-md-4 col-lg-3';
}

// Post Meta
function caspert_post_meta(): void
{
    ?>
    <div class="post-meta d-flex flex-wrap gap-1">
        <div class="post-meta__item post-meta__author">
            <strong class="theme-fw-medium"><?php esc_html_e('Tác giả:', 'caspert'); ?></strong>
        ...
        <div class="post-meta__item post-meta__date">
            <strong class="theme-fw-medium"><?php esc_html_e('Ngày đăng: ', 'caspert'); ?></strong>
            <span><?php echo get_the_date(); ?></span>
        </div>

        <div class="post-meta__item post-meta__comments">
            <?php
            comments_popup_link('0 ' . esc_html__('Bình luận', 'caspert'), '1 ' . esc_html__('Bình luận', 'caspert'), '% ' . esc_html__('Bình luận', 'caspert'));
            ?>
        </div>
    </div>
    <?php
...
// Get Contact Form 7
function caspert_get_form_cf7(): array
{
    $options = array();
    ...
    if (function_exists('wpcf7')) {
        ...
        if (!empty($wpcf7_form_list) && !is_wp_error($wpcf7_form_list)) :
            ...
            $options[0] = esc_html__('Tạo biểu mẫu trước tiên', 'caspert');
        endif;
...
// list social network
function caspert_list_social_network(): array
{
    return array(
        'facebook-f' => 'Facebook',
        'twitter' => 'Twitter',
...
function caspert_get_social_url(): void
{
    $opt_social_networks = caspert_opt(SocialLinkOptions::class)::get_social_list() ?? [];
...
// replace number
function caspert_preg_replace_ony_number($string): string|null
{
    $number = '';
